...
//!
//! Колея железа (параллельная): [`source::SensorSource`] — контракт
//! источника данных, заложен до недели 4, чтобы фичи не срослись с
//! симулятором; сенсорные реализации появятся с прошивками (`firmware/`).
...
/// Контракт источника данных узла: SimSource (хост) / сенсоры (прошивка).
pub mod source;
