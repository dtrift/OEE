...
+ датасет тапов + события ИК-барьера), проигрывает их через три узла
(`oee/line1/{a/status, p/count, q/verdict}`), агрегирует `OEE = A × P × Q`
в `oee/line1/oee` + `tmp/bench/oee_windows.csv` и открывает ratatui-дашборд
