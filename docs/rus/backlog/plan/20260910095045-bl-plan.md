...
> **Статус исполнения (2026-09-10):** план полностью реализован — тесты firmware-workspace зелёные, образы перевыпущены (`firmware/bin/20260910101457-*`), карточки 05/06/32/33/41 перенесены в `../resolved/`. Позже в тот же день карточки 07/31/47 дозакрыты полностью (контракт-тесты в `nodes`/`ml/trainer`/`qemu` с поднятыми `SAMPLE_RATE_HZ`/`TIMESTEPS`, qemu-джоба CI с локально проверенным `PARITY OK`) и также перенесены в `../resolved/`. Отклонения реализации от черновиков ниже:
>
...
- `docs/rus/backlog/resolved/20260909120007-important-features-cli-contract-drift-workspace-bl.md` (firmware-часть)
- `docs/rus/backlog/resolved/20260909120031-important-qemu-not-in-ci-ci-bl.md` (firmware-часть: пин espup + `--release`)
- `docs/rus/backlog/20260909120041-minor-firmware-bl.md`
- `docs/rus/backlog/resolved/20260909120047-rec-firmware-bl.md`
