...
| 2 | 0:50–2:00       | **The twin**: run tab 2. The bench generates the scenario, the three nodes replay it, the aggregator computes OEE = A×P×Q; when the dashboard opens, walk through the gauges (OEE/A/P/Q with the 85%/60% zones), the part counter, the machine status, the verdict ticker. Name the numbers on screen vs the week-5 table.   |
...
- The dashboard needs a terminal ≥ 80×24; if the bench finishes before you
  switch focus, the dashboard still shows the full history (the aggregator
  publishes before it starts).
