...
> **Execution status (2026-09-10):** fully implemented — the firmware workspace tests are green, the images are re-released as `firmware/bin/20260910101457-*`, cards 05/06/32/33/41 moved to `../resolved/`. Later the same day cards 07/31/47 were fully closed as well (contract tests in `nodes`/`ml/trainer`/`qemu` with the promoted `SAMPLE_RATE_HZ`/`TIMESTEPS`, the qemu CI job with a locally verified `PARITY OK`) and also moved to `../resolved/`. Implementation deviations from the sketches below:
>
...
- `docs/eng/backlog/resolved/20260909120007-important-features-cli-contract-drift-workspace-bl.md` (firmware half)
- `docs/eng/backlog/resolved/20260909120031-important-qemu-not-in-ci-ci-bl.md` (firmware half: the espup pin + `--release`)
- `docs/eng/backlog/resolved/20260909120041-minor-firmware-bl.md`
- `docs/eng/backlog/resolved/20260909120047-rec-firmware-bl.md`
