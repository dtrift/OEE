...
## Запас стека (измерено)

Весь остаток DRAM — стек main-задачи (esp-hal 1.2, `ld/sections/stack.x`:
`_stack_start = ORIGIN(RWDATA) + LENGTH(RWDATA)`, с build-time ассертом
минимума). Для release-сборок 2026-09-10: статика заканчивается на
`0x3FC91FE0`, верх `dram_seg` — `0x3FCDB700` — **~294 КиБ стека** против
~25 КиБ пиковой потребности цепочки predict в `firmware-q` (окно 1024 f32
лежит в статической памяти, не на стеке — карточка ревью 20260909120032).
Bring-up: всё равно проверить высокую воду до перехода на реальный I2S
(DMA-буферы добавятся сверху).
