...
Английская версия: [README.md](README.md). План обкатки (сессии Э0–Э7,
гейт): [docs/rus/decompose/firmware.md](../docs/rus/decompose/firmware.md).

Прошивки ESP32-S3 для узлов A/P/Q. Стенд: 2× ESP32-S3-DevKitC-1 (N16R8) —
узлы A и Q; 1× ESP32-S3-WROOM-1 N16R8 **CAM** с OV2640 на борту — узел P
плюс stretch-камера (её разводка камеры занимает часть пинов). Отдельный
workspace — как `fork/microflow`: целевой тулчейн (Xtensa, `espup`) не
должен влиять на хост-CI корневого workspace.
...
| Крейт        | Роль                                                                      |
| ------------ | ------------------------------------------------------------------------- |
| `board`      | Пины стенда по узлам + зарезервированные пины (N16R8; CAM-плата — узел P) |
...
| `firmware-p` | Узел P (CAM-плата): TCRT5000 → фронт + анти-дребезг 50 мс → счёт          |
